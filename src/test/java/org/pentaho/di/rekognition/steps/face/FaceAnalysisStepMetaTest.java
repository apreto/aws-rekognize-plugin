...
public class FaceAnalysisStepMetaTest {
  ...
  public void testGetStepData() {
    FaceAnalysisMeta m = new FaceAnalysisMeta();
    assertEquals( FaceAnalysisData.class, m.getStepData().getClass() );
  }
  ...
  public void testStepAnnotations() {
    ...
    Step stepAnnotation = FaceAnalysisMeta.class.getAnnotation( Step.class );
    ...
    assertEquals( FaceAnalysisMeta.class.getPackage().getName(), stepAnnotation.i18nPackageName() );
    ...
    InjectionSupported injectSupportAnnotation = FaceAnalysisMeta.class.getAnnotation( InjectionSupported.class );
    ...
    for ( Field field : FaceAnalysisMeta.class.getDeclaredFields() ) {
      if ( field.isAnnotationPresent( Injection.class ) || field.isAnnotationPresent( InjectionDeep.class ) ) {
        hasInjection = true;
      }
    }
    for ( Method method : FaceAnalysisMeta.class.getDeclaredMethods() ) {
  ...
  public void testLoadSave() throws KettleException {
     ...
     */
    List<String> attributes = Arrays.asList( "s3BucketName" );
     ...
     */
    LoadSaveTester<FaceAnalysisMeta> tester =
      new LoadSaveTester<FaceAnalysisMeta>( FaceAnalysisMeta.class, attributes );
  ...
  public void testChecks() {
    ...
    for ( CheckResultInterface result : checkResults ) {
      if ( result.getType() == CheckResultInterface.TYPE_RESULT_ERROR
          && result.getText().equals( BaseMessages.getString( FaceAnalysisMeta.class, "FaceAnalysis.CheckResult.ReceivingRows.ERROR" ) ) ) {
        foundMatch = true;
      }
    }
    ...
    for ( CheckResultInterface result : checkResults ) {
      if ( result.getType() == CheckResultInterface.TYPE_RESULT_OK
          && result.getText().equals( BaseMessages.getString( FaceAnalysisMeta.class, "FaceAnalysis.CheckResult.ReceivingRows.OK" ) ) ) {
        foundMatch = true;
      }
    }
    assertTrue( "The step checks should fail if no input fields are given", foundMatch );
